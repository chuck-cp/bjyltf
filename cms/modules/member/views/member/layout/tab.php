...
<ul id="myTab" class="nav nav-tabs" style="margin-bottom: 10px;">
    ...
    <li <? if($action == 'install-information' || $action == 'changescreenlist'):?>class="active"<?endif;?>>
        <?=Html::a('安装信息',['install-information','id'=>$model->id])?>
    </li>
    <li <? if($action == 'led'):?>class="active"<?endif;?>>
        <?=Html::a('LED信息',['led','id'=>$model->id])?>
    </li>
    <li <? if($action == 'bank' || $action == 'combank'):?>class="active"<?endif;?>>
        <?=Html::a('绑定银行卡',['bank','id'=>$model->id])?>
    </li>
