...
$this->endBlock();
$this->title = 'A屏区域排期';
$this->params['breadcrumbs'][] = 'A屏区域排期';
?>
...
<script type="text/javascript">
    $(function(){
        $('ul').on('click','.click',function (){
            ...
            $(this).addClass("cur");
            $.ajax({
                url: '<?=\yii\helpers\Url::to(['address','advert_key'=>'a'])?>',
                type: 'get',
                dataType: 'json',
                data:{'parent_id':id},
        ...
        $('ul').on('click','.select',function(){
           ...
           // alert(area_id);
            ...
            $.ajax({
                url: '<?=\yii\helpers\Url::to(['ascreen'])?>',
                type: 'get',
                dataType: 'json',
                    ...
                    if(phpdata.code==1){
                        ...
                        $.each(phpdata.ProgramListRst,function(k1,v1){
                            ...
                            $.each(v1,function (k, v) {
                                if(v.total_time_sum){
                                    html += '<tr><td><a style="cursor: pointer" class="order" order_id="'+v.order_id+'">'+v.total_time_sum+'s</a></td></tr>';
                                }else{
                                    html += '<tr><td> 0s </td></tr>';
                                }
                            })
