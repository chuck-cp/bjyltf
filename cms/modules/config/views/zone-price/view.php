...
$this->title = '区域价格设置';
$this->params['breadcrumbs'][] = ['label' => '区域价格设置', 'url' => ['zone']];
    ...
    <form class="form-inline col-md-3">
        <div class="form-group">
            ...
            <div class="input-group">
                <div class="input-group-addon">店铺价格：</div>
                <input type="text" id="price" class="form-control" placeholder="请输入店铺价格" value="<?=Html::encode($data['regionalprice'])?>">
                <div class="input-group-addon">元</div>
            </div>
            <div class="input-group">
                <div class="input-group-addon">每月补助：</div>
                <input type="text" id="month_price" class="form-control" placeholder="请输入每月补助" value="<?=Html::encode($data['subsidyprice'])?>">
                <div class="input-group-addon">元</div>
            </div>
        </div>
    </form>
...
<div class="row">
    <ul class="left zone sybox">
        <? foreach ($arr as $k => $v):?>
            <li>
<!--                --><?//=Html::encode(SystemAddress::getAreaNameById($k))?>
                <? foreach ($v as $kt => $vt) :?>
        ...
        $('.sub .ck').click(function () {
         ...
         /*   var price_id = $('.hidden').attr('id');*/
            ...
            $.ajax({
                ...
                type : 'get',
                dataType : 'json',
                data : {'price': price,'month_price':mprice,'id':id},
