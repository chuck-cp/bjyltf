...
<div class="system-startup-form aa" >
    ...
    <input type="hidden" name="filename" value="back-stage">
        ...
        <div class="col-sm-3 upload" id="aaaaaa" style="margin:0 0 20px -14px; ">
        </div>
...
<script type="text/javascript">
    $(function(){
        $(".submit").click(function(){
            ...
            if(!content){
                layer.msg('请先上传视频资料',{icon:2});
                return false;
            }
            var data=$('#w0').serialize();
            $.ajax({
                url:'<?=\yii\helpers\Url::to(['createvideo'])?>',
                type : 'POST',
                dataType : 'json',
                data : data,
        ...
        $('#uploadVideoNow-file').on('change', function (e) {
            ...
            if(size>104857600){
                layer.msg('视频大小不能超过100M',{icon:2});
                return false;
            }
            if(type!=='video/mp4'){
                layer.msg('视频格式必须为Mp4',{icon:2});
                return false;
            }

           // console.log(this.files);return false;
            ...
            var resultMsg = qcVideo.ugcUploader.start({
                ...
                finish: function(result){
                    ...
                    if(result.fileId){
                        $("#aaaaaa").html('<video controls  width="480" height="260" ><source src='+result.videoUrl+' type="video/mp4" />  </video>');
                    }
                }
            });
            if(resultMsg){
