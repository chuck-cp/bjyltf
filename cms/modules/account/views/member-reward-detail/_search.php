    ...
    <div class="row">
        <table class="grid table table-striped table-bordered search">
            <tr>
                ...
                <td class="date">
                    <p>完成时间</p>
                    <?=$form->field($model,'finish_at')->textInput(['class'=>'form-control datepicker fm collection-width','placeholder'=>'开始时间'])->label(false);?>
                </td>
                <td class="date" colspan="7">
                    <?=$form->field($model,'finish_at_end')->textInput(['class'=>'form-control datepicker fm mtop22 collection-width','placeholder'=>'结束时间'])->label(false);?>
                </td>
            </tr>
            <tr>
                <td>
                    <p>店铺类型</p>
                    <?php  echo $form->field($model, 'search_shop_type')->dropDownList(['2'=>'总部','1'=>'自营/租赁'],['key'=>'shop_type','class'=>'form-control shop_type fm'])->label(false) ?>
                </td>
                <td>
                    <p>法人ID</p>
                    <?=$form->field($model,'shop_member_id')->textInput(['class' => 'form-control collection-width fm'])->label(false); ?>
                </td>
                <td>
                    <p>法人姓名</p>
                ...
                <td id="shop_type">
                    <p>总部名称</p>
                    <?=$form->field($model,'company_name')->textInput(['class' => 'form-control collection-width fm'])->label(false); ?>
                </td>
                <td>
                    <p>所属省</p>
...
<script type="text/javascript">
    $(function () {
        ...
        $('.shop_type').change(function(){
            var options=$("#memberrewarddetailsearch-search_shop_type option:selected"); //获取选中的项
            var type = options.val(); //拿到选中项的值
            if(type==1){
                $("#shop_type").hide();
                $("#memberrewarddetailsearch-company_name").val("");
            }else{
                $("#shop_type").show();
            }
        })
