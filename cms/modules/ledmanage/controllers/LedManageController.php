...
use cms\modules\ledmanage\models\search\SystemDeviceSearch;
...
use ZipArchive;
/**
 * LedManageController implements the CRUD actions for SystemDevice model.
 */
class LedManageController extends CmsController
{
    /**
     * @inheritdoc
     */
    ...
    {
        $searchModel = new SystemDeviceSearch();
        ...
        if(isset($arr['search']) && $arr['search'] == 0){
            ...
            if($DataCount==0){
                return $this->render('index', [
                    ...
                    'asArr'=>$asArr,
                    'isoutput'=>SystemDevice::isoutput()
                ]);
            }
            //取得最大页数
            ...
            $j=0;
            //表头
            $title=['序号','硬件编号','软件编号','厂家名称','办事处','仓库','规格','GPS','入库时间','入库负责人','出库时间','出库负责人','设备领取人','是否出库','批次','备注'];
            for($i=1;$i<=$count;$i++){
    ...
    public function actionOffices($kuid='')
    {
        ...
        if(isset($arr['search']) && $arr['search'] == 0){
            ...
            for($i=1;$i<=$count;$i++){
                ...
                $j=$i*3000;
                //处理csv要导出的数据
                $Csv=SystemDevice::CsvData($data,$asArr_2);
                $file_name="Led_".time().'_'.$i.".csv";
                //生成CSV文件并返回文件路径
                $filenameArr[]=ToolsClass::Getcsvzip($Csv,$title,$file_name);
            }
        ...
        return $this->render('offices', [
            ...
            'asArr'=>$asArr,
            'kuid'=>$kuid,
            'isoutput'=>SystemDevice::isoutput($kuid)
        ]);
    }

    /**
     * Displays a single SystemDevice model.
     * @param string $id
     * @return mixed
    ...
    public function actionOutPut(){
        ...
        $model = SystemDevice::findOne(['id'=>$data['deviceid']]);
     ...
     * 单个出办事处页面
     */
    public function actionSingleOffices($deviceid,$kuid){
        $offices = SystemOffice::find()->where(['<>','id',$kuid])->asArray()->all();
        return $this->renderPartial('single-offices', [
            'kuid'=>$kuid,
            'offices' => $offices,
            'deviceid'=>$deviceid,
        ]);
    }
    public function actionOutPutOffice(){
    $data=Yii::$app->request->post();
    $model = SystemDevice::findOne(['id'=>$data['deviceid']]);
    if($model && $model->getAttribute('is_output') !== 1){
        $model->is_output = 1;
        $model->stock_out_at = date('Y-m-d : H:i:s');
        $model->out_manager = Yii::$app->user->identity->getId();
        $model->receive_office_id = $data['kuid'];
        $re = $model->save();
        if($re){
            $offices = SystemOffice::find()->where(['id'=>$data['kuid']])->asArray()->one();
            LogDevice::addlog($model->device_number,$offices,2,2);
        }
        return $re == true ? 1 : 0;
    }
    return 0;
    ...
    public function actionCheckInfo(){
        ...
        $numarray['emptynum'] = [];
        $numinfo = SystemDevice::find()->where(['and',['in','device_number',$array['deviceid']],['office_id'=>$array['kuid']]])->select('id,device_number,is_output')->asArray()->all();
    ...
    public function actionLot(){
        try{
            ...
            if(count($ids['deviceid']) > 0){
                SystemDevice::updateAll(['is_output' => 1,'stock_out_at'=>date('Y-m-d : H:i:s'),'out_manager'=>Yii::$app->user->identity->getId(),'receive_member_id'=>$memberid['id']],['device_number' => $ids['deviceid']]);
    ...
    public function actionLotku(){
        try{
            ...
            if(count($ids['deviceid']) > 0){
                SystemDevice::updateAll(['is_output' => 1,'stock_out_at'=>date('Y-m-d : H:i:s'),'out_manager'=>Yii::$app->user->identity->getId(),'receive_office_id'=>$ids['tokuid']],['device_number' => $ids['deviceid']]);
                foreach($ids['deviceid'] as $knum=>$vnum){
    ...
    public function actionScreenInfo($did,$device_number){
        ...
        /*if(!$shop){
            echo '<h2 style="margin-top: 30%;text-align: center;letter-spacing: 14px;">暂无信息</h2>';
            return false;
        }*/
        ...
        $logisticsList = SystemDevice::getWlInfo($logistics['name'], $logistics['logistics_id']);
    ...
    /**
     * Updates an existing SystemDevice model.
    ...
    /**
     * Deletes an existing SystemDevice model.
    ...
    public function actionCheckUnique(){
        ...
        if($number){
            $isExist = SystemDevice::findOne(['device_number' => $number]);
            if($isExist !== null){
                return false;
            }
            return true;
    ...
    public function actionCheckChange(){
        ...
        if(!empty($number) && !empty($kuid)){
            $isExist = SystemDevice::findOne(['device_number' => $number,'receive_office_id'=>$kuid,'is_output'=>1]);
            if($isExist !== null){
                return true;
            }
            return false;
     ...
     * 入库检测软件编号的唯一性
     */
    public function actionCheckSoft(){
        $number = Yii::$app->request->post('number') ;
        if($number){
            $isExist = SystemDevice::findOne(['software_id' => $number]);
            if($isExist !== null){
                return false;
            }
            return true;
     ...
     *出库检测设备号的唯一性
     */
    public function actionCheckque(){
        $number = Yii::$app->request->post('number');
        if($number){
            $isExist = SystemDevice::findOne(['device_number' => $number]);
            if($isExist !== null){
    ...
    public function actionStorehouse(){
        ...
        $StorehouseAll = SystemOffice::find()->where(['id'=>$id])->select('storehouse')->asArray()->one();
        return  json_encode(explode(',',$StorehouseAll['storehouse']));
    }
    /**
     * Finds the SystemDevice model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param string $id
     * @return SystemDevice the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */

    protected function findModel($id)
    {
        if (($model = SystemDevice::findOne($id)) !== null) {
            return $model;
        }
