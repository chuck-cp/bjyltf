...
//            'action' => ['/ledmanage/led-manage/lot'],
        ...
        <hr>
        <h3 style="margin-left: 10px;">设备编号：<span style="font-size: 16px;">设备硬件编号(包装盒编码)</span></h3>
        <div id="number">
            <div class="add">
...
<script type="text/javascript">
    //自动加载内部人员名单
    $(function(){
//        member =$("#membersearch-name").val();//制定领取人
//        deviceid = [];
        var datas = <?=$nmember?>;
        $("#systemdevice-receive_member_id").autocomplete({source: datas});
    });
    ...
    //检查所填写的订单有无重复
    $(".sub").click(function () {
        var kuid = $('input[name="SystemDevice[office_id]"]').val();//当前办事处
        ...
        $.ajax({
            ...
            success:function (resdata) {
                if(resdata ==true){
                    $.ajax({
                        url:'<?=\yii\helpers\Url::to(['/ledmanage/led-manage/lotku'])?>',
                        type : 'POST',
                        dataType : 'json',
