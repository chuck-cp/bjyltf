...
//        deviceid = [];
        var datas = <?=$nmember?>;
        $("#systemdevice-receive_member_id").autocomplete({source: datas});//制定领取人
    });
    ...
    $(".sub").click(function () {
        ...
        var member =$("#systemdevice-receive_member_id").val();//制定领取人
        if(member==''){
            layer.alert('请填写领取人员');
            return false;
        }
        ...
        $.ajax({
            ...
            success:function (resdata) {
                if(resdata ==true){
                    $.ajax({
                        url:'<?=\yii\helpers\Url::to(['/ledmanage/led-manage/lot'])?>',
                        type : 'POST',
                        dataType : 'json',
