...
<?$officeall = \cms\models\SystemOffice::find()->asArray()->all();?>
...
//            'action' => ['/ledmanage/led-manage/lot'],
            'method' => 'post',
        ]); ?>
        <?= $form->field($model, 'office_id')->hiddenInput(['value'=>$kuid])->label(false); ?>
        <div class="form-group" style="margin-top: 10px;margin-left: 15px;">
            <label class="control-label" for="systemdevice-office_id">调库至: </label>
            <select id="device" class="kunum" name="kunum" style="border:none;font-size: 25px;"/>
            <?foreach($officeall as $ko=>$vo):?>
                <?if($vo['id'] != $office->id):?>
                <option value="<?=Html::encode($vo['id'])?>"><?=Html::encode($vo['office_name'])?></option>
                <?endif;?>
            <?endforeach;?>
            </select>
        </div>
        ...
        <div id="number">
            <div class="add">
                <div class=" mtop" style="display: flex">
                    <div class="col-md-3">
                        <?= $form->field($model, 'device_number[]')->textInput(['placeholder'=>'画框编号','class'=>'form-control  mleft'])->label(false); ?>
                    </div>
                    <div class="col-md-3">
                        <?= $form->field($model, 'device_number[]')->textInput(['placeholder'=>'画框编号','class'=>'form-control  mleft'])->label(false); ?>
                    </div>
...
<script type="text/javascript">
    ...
    $('.keep').bind('click',function () {
        var adhtml = '';
            adhtml += '<div class=" mtop" style="display: flex"><div class="col-md-3"><div class="form-group field-systemdevice-device_number"><input type="text" id="systemdevice-device_number" class="form-control  mleft" name="SystemDevice[device_number][]" placeholder="画框编号"></div></div><div class="col-md-3"><div class="form-group field-systemdevice-device_number"><input type="text" id="systemdevice-device_number" class="form-control  mleft" name="SystemDevice[device_number][]" placeholder="画框编号"></div></div><div class="col-md-3"><div class="form-group field-systemdevice-device_number"><input type="text" id="systemdevice-device_number" class="form-control mleft" name="SystemDevice[device_number][]" placeholder="画框编号"></div></div><div class="col-md-3"><div class="form-group field-systemdevice-device_number"><input type="text" id="systemdevice-device_number" class="form-control mleft" name="SystemDevice[device_number][]" placeholder="画框编号"></div></div></div><div class=" mtop" style="display: flex"><div class="col-md-3"><div class="form-group field-systemdevice-device_number"><input type="text" id="systemdevice-device_number" class="form-control  mleft" name="SystemDevice[device_number][]" placeholder="画框编号"></div></div><div class="col-md-3"><div class="form-group field-systemdevice-device_number"><input type="text" id="systemdevice-device_number" class="form-control  mleft" name="SystemDevice[device_number][]" placeholder="画框编号"></div></div><div class="col-md-3"><div class="form-group field-systemdevice-device_number"><input type="text" id="systemdevice-device_number" class="form-control mleft" name="SystemDevice[device_number][]" placeholder="画框编号"></div></div><div class="col-md-3"><div class="form-group field-systemdevice-device_number"><input type="text" id="systemdevice-device_number" class="form-control mleft" name="SystemDevice[device_number][]" placeholder="画框编号"></div></div></div>';
    ...
    $(".sub").click(function () {
        ...
        $.ajax({
            ...
            success:function (resdata) {
                if(resdata ==true){
                    $.ajax({
                        url:'<?=\yii\helpers\Url::to(['/ledmanage/frame/lotku'])?>',
                        type : 'POST',
                        dataType : 'json',
                        data : {'deviceid':numids,'tokuid':tokuid},
                    ...
                    layer.open({
                        ...
                        anim: 2,
                        shadeClose: true, //开启遮罩关闭
                        content: '<div style="width: 200px;height: 300px;">未入库设备：<\/br>'+resdata.emptynum.join(',<\/br>')+'<\/br>已出库设备：<\/br>'+resdata.outputnum.join(',<\/br>')+'<\/div>'
                    });
                    return false;
                }
            },
