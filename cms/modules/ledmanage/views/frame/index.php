...
$this->title = '设备列表';
$this->params['breadcrumbs'][] = '库存管理';
$this->params['breadcrumbs'][] = '画框设备列表';
...
<!--        <span>搜索结果共计： --><?//=Html::encode($asArr['counts'])?><!-- 条</span>-->
        <?php foreach ($asArr['data'] as $k=>$v):?>
            <tr>
                <td><?echo $v['id'];?></td>
                <td><?echo $v['device_number'];?></td><!--硬件编号-->
                <td><?echo array_key_exists($v['manufactor'],$asArr['config']['frame_device_manufactor']) ? $asArr['config']['frame_device_manufactor'][$v['manufactor']] : '未设置';?></td><!--厂家名称-->
                <td>
                    <?if(!empty($v['offices'])):?>
                ...
                <td><?echo array_key_exists($v['device_size'],$asArr['config']['frame_device_size']) ? $asArr['config']['frame_device_size'][$v['device_size']] : '未设置';?></td><!--规格-->
                ...
                <td><?echo SystemDeviceFrame::getIsHave('nfc',$v['nfc']);?></td><!--nfc-->
                <td><?echo $v['create_at'];?></td><!--入库时间-->
                <td>
                ...
                <td><?echo $v['stock_out_at'];?></td><!--出库时间-->
                <td>
                ...
                <td><?echo SystemDevice::getIsHave('is_output',$v['is_output']);?></td><!--出库-->
                <td><?echo $v['batch']?></td><!--批次-->
                <td>
